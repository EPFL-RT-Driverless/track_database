...
class WorldElement:
    ...
    def to_dict(self) -> dict:
        return {
            "include": {"uri": self._model_uri, "name": self._name, "pose": self._pose}
        }
...
class TrackParser:
    ...
    def _extract_cone_information(self):
        for cone_type in self.cone_types:
            for index, cone in (
                self.cones[self.cones.cone_type == cone_type].reset_index().iterrows()
            ):
    ...
    def _to_xml(self):
        self._xml = "\n".join(
            [dict2xml(element, indent=" ") for element in self.elements]
        )

    def _wrap_xml(
        self, wrapper_name: str, special_tag: str, special_tag_value: str
    ) -> str:
